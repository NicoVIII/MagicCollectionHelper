    ...
    inventoryTests [
        fitsRulesTests [
            ...
            <| (fun () ->
                ...
                let sets =
                    [ [], false; [ m21 ], true; [ rna ], false; [ rna; m21 ], true ]
            ...
            <| (fun () ->
                ...
                for (set, expected) in sets do
                    let rules = Rules.empty |> Rules.withColorIdentity set
            ...
            <| (fun () ->
                let sets =
                    [ [], false; [ Rare ], true; [ Uncommon ], false; [ Uncommon; Rare ], true ]
                    |> List.map (fun (l, r) -> Set.ofList l, r)
            ...
            <| (fun () ->
                let sets =
                    [ [], true; [ "Land" ], true; [ "Creature" ], false ]
                    |> List.map (fun (l, r) -> Set.ofList l, r)

                for (set, expected) in sets do
                    let rules = Rules.empty |> Rules.withTypeContains set
            ...
            <| (fun () ->
                let sets =
                    [ [], true; [ "Land" ], false; [ "Creature" ], true ]
                    |> List.map (fun (l, r) -> Set.ofList l, r)

                for (set, expected) in sets do
                    let rules = Rules.empty |> Rules.withTypeNotContains set
